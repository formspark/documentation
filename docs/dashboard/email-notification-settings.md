...
| `sanitizeUrl`                 | Returns the given URL only if it uses a safe scheme (`http(s)`, `mailto`, `tel`, a relative path, or an anchor); otherwise returns `#`. Use it around links built from submitted data so an unsafe value can't render a clickable exploit. | <span v-pre>`{{sanitizeUrl viewLink}}`</span> → `https://example.com` (or `#` if unsafe)                                                                                                    |
